...
void spmv_serial(int m, int *RowPtr, int *ColIdx, VALUE_TYPE *Val, VALUE_TYPE *Y, VALUE_TYPE *X)
{
    for (i = 0; i < m; i++) 
    {
        ...
        for (j = RowPtr[i]; j < RowPtr[i+1]; j++)
        {
            ...
            //printf("Y[%d]=Val[%d]%.2f*X[%d]%.2f\n",i,j,Val[j],ColIdx[j],X[ColIdx[j]]);
        }
        //printf("Y[%d]=%.2f\n",i,Y[i]);
    }
}
...
int dividematrixbyrow(struct MATRIX sourcematrix, struct MATRIX *mtx, int partnum)
{
    ...
    {
        mtx[i].colnum = sourcematrix.colnum / partnum;
    }
    mtx[partnum-1].colnum = sourcematrix.colnum - (sourcematrix.colnum/partnum)*(partnum-1);*/
    int start, end;
    for (i = 0; i < partnum; i++)
    {
        start = subrowadd[i];
...
int main(int argc, char ** argv)
{
    ...
    if (id == 0)
    {
        ...
        for(int i=0;i<sourcematrix.nnznum;i++) sourcematrix.val[i]=1;
        //create input vector
	...
	*/
        //compute y_golden
        // VALUE_TYPE *y_golden = (VALUE_TYPE *)malloc(sizeof(VALUE_TYPE) * sourcematrix.colnum);
        // spmv_serial(&sourcematrix, sourcevector, y_golden);
        // vec_reorder(y_golden, reorder, parts, mtx.colnum);
        ...
        { 
            for (j = 0; j < mtx[i].rownum; j++)
            {
                for (k = mtx[i].rowptr[j]; k < mtx[i].rowptr[j+1]; k++)
                {
                    ...
                    if (mtx[i].colidx[k] < xidx[i] || mtx[i].colidx[k] >= xidx[i+1])
                    {
                        ...
                        //printf("%d %d\n", mtx[i].colidx[k], comm[need1].flag[mtx[i].colidx[k]]);
            ...
            //for (j = 0; j < sourcematrix.colnum; j++) comm[i].flag[j] = 0;
            //memset(comm[i].flag, 0, sourcematrix.colnum);
        }
        printf("communicate_total_num %d\n", communicate_total_num);

        //create MPI_Type
        /*int blockLength[3] = {1, comm[1].infocount, comm[1].infocount};
        MPI_Aint infocount_p, recvid_p, index_p;
        MPI_Get_address((void *)comm[1].infocount, &infocount_p);
        MPI_Get_address((void *)comm[1].recvid, &recvid_p);
        MPI_Get_address((void *)comm[1].index, &index_p);
        MPI_Aint displacement[3] = {0, recvid_p-infocount_p, index_p-recvid_p};
        int type[3] = {MPI_INT, MPI_INT, MPI_INT};
        MPI_Datatype newType;
        MPI_Type_create_struct(3, blockLength, displacement, type, &newType);
        MPI_Type_commit(&newType);

        //send MPI_Type
        MPI_Send(&comm[1].infocount, 1, newType, 1, 0, MPI_COMM_WORLD);
        for (i = 1; i < p; i++)
        {
            MPI_Send(&comm[i].infocount, 1, MPI_INT, 1, 8, MPI_COMM_WORLD);
            MPI_Send(comm[i].recvid, comm[i].infocount, MPI_INT, 1, 9, MPI_COMM_WORLD);
            MPI_Send(comm[i].index, comm[i].infocount, MPI_INT, 1, 10, MPI_COMM_WORLD);
        }*/
        ...
        //printf("send submatrix time %4.2f ms\n", (t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0);
    } 
    else
    {
    ...
    for (j = 0; j < vec_commun.infocount; j++)
    {
        ...
        //printf("id %d vec_commun.index[%d] %d  value[%d] %4.2f\n", id, j, vec_commun.index[j], j, x[vec_commun.index[j]]);
    }
    ...
    for (int r = 0; r < NTIMES; r++)
    {
        ...
        {
            gettimeofday(&t2, NULL);
            comtime += ((t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0);
        }
    }
/*
    {
        int sum_mesg=0;
        for(int i=0;i<p;i++) {
            printf("进程 %d 接收 进程 %d 的通信量为 %d\n",id,i,length[i]);
            sum_mesg+=length[i];
        }
        int sum_local=0,sum_remote=0;
        for(int i=0;i<submatrix.rownum;i++) {
            for(int j=submatrix.rowptr[i];j<submatrix.rowptr[i+1];j++) {
                int m=submatrix.colnum/p;
                int l=m*id,r=(id==p-1?submatrix.colnum:m*(id+1));
                //printf("进程 %d , col %d , l %d , r %d\n",id,submatrix.colidx[i],submadd[id],submadd[id+1]);
                if(submatrix.colidx[j]>=l&&submatrix.colidx[j]<=r) 
                    sum_local++;
                else sum_remote++;
            }
        }
        printf("进程 %d 接收的通信量和为 %d , 本地矩阵计算量为 %d , 远程矩阵计算量为 %d\n",id,sum_mesg,sum_local,sum_remote);
    }
*/
    for (i = 0; i < p; i++)
    ...
    for (int q = 0; q < NTIMES; q++)
    {
        ...
        {
            gettimeofday(&t2, NULL);
            distime += ((t2.tv_sec - t1.tv_sec) * 1000.0 + (t2.tv_usec - t1.tv_usec) / 1000.0);
        }
    // for (int q = 0; q < NTIMES; q++)
    // {
    //     #pragma omp parallel for
    ...
    if (id == 0)
    {
        ...
        printf("spmv time %4.2f ms\n",distime);
	    printf("communicate time + spmv time %4.2f ms  %4.2f GFlops\n", distime+comtime, GFlops);
        writeresults("mpi+omp.csv", filename, sourcematrix.rownum, sourcematrix.colnum, sourcematrix.nnznum,comtime,distime, distime+comtime, GFlops, p, nthreads);
    }
